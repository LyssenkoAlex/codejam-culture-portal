export const studentsInfo = [
  {
    id: 1.1,
    student: 'Alexandr Lyssenko',
    task: 'Basic project structure',
    time: '4 hours',
  },
  {
    id: 1.2,
    student: 'Alexandr Lyssenko',
    task: 'Redux and basic actions implement',
    time: '6 hours',
  },
  {
    id: 1.3,
    student: 'Alexandr Lyssenko',
    task: 'Portal navigation via routing',
    time: '5 hours',
  },
  {
    id: 1.4,
    student: 'Alexandr Lyssenko',
    task: 'Implement material design for header and directors',
    time: '5 hours',
  },
  {
    id: 1.5,
    student: 'Alexandr Lyssenko',
    task: 'Worklog page design',
    time: '5 hours',
  },
  {
    id: 3.1,
    student: 'Alexey Kostsevich',
    task: 'Page with a list of authors on the given topic with search widget',
    time: '15 hours',
  },
  {
    id: 3.1,
    student: 'Alexey Kostsevich',
    task: 'Table with hours and time for implementing functionality',
    time: '0,4 hours',
  },
  {
    id: 4.1,
    student: 'Aleksandr Serikov',
    task: 'Create the basic structure of the project',
    time: '0.5 hours',
  },
  {
    id: 4.2,
    student: 'Aleksandr Serikov',
    task: 'Create navigation and menu of language',
    time: '4 hours',
  },
  {
    id: 4.3,
    student: 'Aleksandr Serikov',
    task: 'Fix menu of language',
    time: '0.5 hours',
  },
  {
    id: 5,
    student: 'Stanislav Iosifov',
    task: 'Creating director page(works, biography, photo, video, map)',
    time: '15 hours',
  },
  {
    id: 5.1,
    student: 'Stanislav Iosifov',
    task: 'Creating team page.',
    time: '4 hours',
  },
  {
    id: 5.2,
    student: 'Stanislav Iosifov',
    task: 'Code refactor',
    time: '2 hours',
  },
  {
    id: 5.3,
    student: 'Stanislav Iosifov',
    task: 'Adding and changing styles',
    time: '4 hours',
  },
  {
    id: 6.1,
    student: 'Siarhei Karukhin',
    task: 'Create Skype and Telegram groups, organization of the first call',
    time: '1 hours',
  },
  {
    id: 6.2,
    student: 'Siarhei Karukhin',
    task: 'Create worklog page',
    time: '4 hours',
  },
  {
    id: 6.3,
    student: 'Siarhei Karukhin',
    task: 'Start to do page with a list of directors (not implemented)',
    time: '2 hours',
  },
  {
    id: 6.4,
    student: 'Siarhei Karukhin',
    task: 'Create slider for the main page (not implemented)',
    time: '2 hours',
  },
  {
    id: 7.1,
    student: 'Vitali Himzheuski',
    task: 'Collection of information about Directors, translation into three languages',
    time: '4 hours',
  },
  {
    id: 7.2,
    student: 'Vitali Himzheuski',
    task: 'Create data.json and put there information about Directors',
    time: '2 hours',
  },
  {
    id: 7.3,
    student: 'Vitali Himzheuski',
    task: 'The main page (title, description, director of the day)',
    time: '3 hours',
  },
  {
    id: 7.4,
    student: 'Vitali Himzheuski',
    task: 'Translate menu',
    time: '0.25 hours',
  },
];



export const MIN = [
  {
    id: 1,
    checked: '1',
    score: 10,
    achieved: 10,
    description: 'Main page + page with a list of authors + author`s page (only pages with content without widgets)',
  },
  {
    id: 2,
    checked: '0',
    score: 10,
    achieved: 10,
    description: 'Page with team members + page with worklog',
  },
  {
    id: 3,
    checked: '1',
    score: 10,
    achieved: 10,
    description: 'Page with list of authors contains search widget',
  },
  {
    id: 4,
    checked: '0',
    score: 20,
    achieved: 20,
    description: 'Portal has two languages',
  }
];

export const NORMAL = [
  {
    id: 1,
    checked: '1',
    score: 20,
    achieved:5,
    description: 'Portal has page with styleguide',

  },
  {
    id: 2,
    checked: '0',
    score: 10,
    achieved: 5,
    description: 'Mobile version is okey',
  },
  {
    id: 3,
    checked: '1',
    score: 10,
    achieved: 7,
    description: 'Ipad/tablet version is okey',
  },
  {
    id: 4,
    checked: '0',
    score: 10,
    achieved: 10,
    description: 'Authors page contains timeline',
  },
  {
    id: 5,
    checked: '1',
    score: 10,
    achieved: 10,
    description: 'Authors page contains video overlay',
  },
  {
    id: 6,
    checked: '0',
    score: 20,
    achieved: 20,
    description: 'Authors page contains photo gallery',
  },
  {
    id: 7,
    checked: '1',
    score: 10,
    achieved: 10,
    description: 'Authors page contains map (geowidget)',
  },
  {
    id: 8,
    checked: '0',
    score: 20,
    achieved: 10,
    description: 'Design (typography, icons, colors, links + buttons + input are styled)',
  },
  {
    id: 9,
    checked: '1',
    score: 20,
    achieved: 20,
    description: 'Material-ui / bootstrap is used',
  },
  {
    id: 10,
    checked: '0',
    score: 10,
    achieved: 10,
    description: 'Portal has third language',
  }
];

export const EXTRA = [
  {
    id: 1,
    checked: '1',
    score: 10,
    achieved: 10,
    description: 'Confidence of the project presentation',
  },
  {
    id: 2,
    checked: '0',
    score: 10,
    achieved: 10,
    description: 'Project is made using gatsbyjs',
  },
  {
    id: 3,
    checked: '1',
    score: 10,
    achieved: 0,
    description: 'Contentful / netlify cms is used for content management',
  },
  {
    id: 4,
    checked: '0',
    score: 20,
    achieved: 0,
    description: 'Animations / special effects like paralax',
  },
  {
    id: 5,
    checked: '1',
    score: 20,
    achieved: 10,
    description: 'Outstanding design',
  },
  {
    id: 6,
    checked: '0',
    score: 20,
    achieved: 0,
    description: 'Storybook/styleguidist/other react styleguide tool usage for the page with styles',
  }
  ];

export const FINES = [
  {
    id: 1,
    checked: '0',
    score: -10,
    achieved: 0,
    description: 'if there are less than 5 commits from each active team member. Everyone should merge their own PRs',
  },
  {
    id: 2,
    checked: '0',
    score: -50,
    achieved: 0,
    description: 'up to -50 points for violations stage2-tasks-requirements',
  },
  {
    id: 3,
    checked: '0',
    score: -40,
    achieved: 0,
    description: ' if there is no worklog for team',
  },
  {
    id: 4,
    checked: '0',
    score: -20,
    achieved: 0,
    description: 'too primitive (ugly for 2020) design / UX',
  }
];

export const PROBLEMS = [
  {
    id: 1,
    desc: 'Двойная" работа. Из-за первоначальной несработанности - два человека выполняли одну и ту же задачу.'
  },
  {
    id:2,
    desc: 'Мерж без пула. Возникали моменты, когда перед мержем не подтягивались актуальные изменения.'
  },
  {
    id:3,
    desc:'Разный временной пояс. Общие созвон для одних был во время, а для других был шагом объятья Морфея'
  },
  {
    id:4,
    desc:'Определение общего стиля дизайна и согласованности компонентов'
  },

];


